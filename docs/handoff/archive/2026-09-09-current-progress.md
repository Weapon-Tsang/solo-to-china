# Historical baseline — superseded by the 2026-09-12 upgrade
...
- Real Playground REST checks cover Contract `2.1.0`, Registry `1.3.0`, Theme `0.32.1`, four taxonomy-only guide types, 26 CMS capabilities, the generated Publish Package Schema, authenticated draft ingestion, exact-order serialization, idempotency, rejection cases, and cache headers.
...
- Survival Kit strip.
- Survival Kit landing page is now a five-part pre-departure field manual. Payment, Essential Apps, eSIM and mobile data, Visa and Entry, and Internet Access each include three setup checks, a fallback plan, a clear readiness test, and relevant official references.
- Homepage Survival Kit shortcuts deep-link to the matching field-manual section instead of sending every topic to the top of the hub.
...
- The Planner landing page leads with a high-impact AI itinerary promise, a single vermilion `Generate my AI itinerary` action, an illustrative day-by-day result preview, and a concise three-step explanation across desktop and mobile.
- Planner removes its competing Share action and repetitive affiliate sentence while preserving the site-wide homepage Planner card.
- FAQ now groups 12 substantive answers into Entry and Documents, Money and Phone, Apps and Internet, and Getting Around and Staying Safe, with a sticky desktop topic rail and horizontally scrollable mobile topic chips.
- FAQ answers explain the condition that changes the recommendation, the practical fallback, a related internal next step, and official-source links for entry, payments, rail, and accommodation registration.
- Homepage FAQ answers now provide useful summaries and deep-link to the relevant FAQ or Survival Kit section instead of using one-line placeholders.
- Footer now uses solid deep ink, real email and WhatsApp destinations, clear Explore/Tools/Help/About groups, and two-column mobile navigation. Low-value Affiliate Disclosure/Disclaimer links, the duplicated Affiliate Disclosure legal link, and the principles slogan were removed from the Footer; their standalone pages remain available.
...
- Planner band.
- Homepage and Planner-page CTAs use the shared `stc_get_trip_planner_url()` helper and open the approved affiliate destination at `https://www.trip.com/t/bCPFQ85ZHW2` with `sponsored nofollow noopener noreferrer`, preventing SEO weight transfer.
...
Current Child Theme version: `0.11.1`.
...
- Component Registry `1.3.0`, Content Contract `2.1.0`, Parent Theme `0.32.1`, and Child Theme `0.11.1` publish 26 CMS capabilities, including 23 ordered page blocks.

...
function stc_tools_rate_limit( $bucket, $limit, $window ) {
	$key = 'stc_tool_' . sanitize_key( $bucket ) . '_' . substr( stc_tools_client_key(), 0, 32 );
...
function stc_tools_validate_image_upload( $file ) {
	...
	if ( in_array( (int) $file['error'], array( UPLOAD_ERR_INI_SIZE, UPLOAD_ERR_FORM_SIZE ), true ) ) {
		...
		return new WP_Error( 'server_upload_limit', __( 'The server rejected this large photo. Ask the site administrator to raise the PHP upload limits.', 'solo-to-china-tools' ), array( 'status' => 413 ) );
	}
...
function stc_tools_rest_place_finder( WP_REST_Request $request ) {
	...
	if ( empty( $files ) && $content_length > 0 ) {
		return new WP_Error( 'server_upload_limit', __( 'The server rejected these photos before WordPress could read them. Ask the site administrator to raise the PHP upload limits.', 'solo-to-china-tools' ), array( 'status' => 413 ) );
	}
	...
	$raw = $provider->identify( $images, array( 'city_hint' => $city_hint ) );
	unset( $images );
	if ( is_wp_error( $raw ) ) {
...
function stc_tools_rest_taxi_card( WP_REST_Request $request ) {
	...
	if ( $provider instanceof STC_Place_Resolver_Provider && $provider->is_available() ) {
		$raw = $provider->resolve( $query );
		if ( ! is_wp_error( $raw ) ) {
			$result = stc_tools_prepare_vision_result( $raw );
...
function stc_tools_protect_rest_response( $response, $server, $request ) {
	...
	if ( 0 === strpos( $route, '/stc/v1/place-finder' ) || 0 === strpos( $route, '/stc/v1/taxi-card' ) ) {
		$response->header( 'Cache-Control', 'private, no-store, max-age=0' );
		$response->header( 'X-Content-Type-Options', 'nosniff' );
	}
	return $response;

...
Assert-Runtime ($Contract.theme_version -eq "0.33.7") "Content Contract Theme version is not 0.33.7."
...
foreach ($RemovedFooterSlug in @("affiliate-disclosure", "disclaimer")) {
    $FooterHtml = [regex]::Match($SurvivalHtml, '<footer\b[\s\S]*?</footer>', 'IgnoreCase').Value
    Assert-Runtime (-not $FooterHtml.Contains("/$RemovedFooterSlug/")) "Footer still exposes removed low-value link: $RemovedFooterSlug"
}
...
foreach ($GalleryFoundationToken in @("Tokens and primitives", "Foundation sets", "stc-component-gallery__foundations", "stc-button--outline", "stc-badge--success")) {
    Assert-Runtime ($GalleryHtml.Contains($GalleryFoundationToken)) "Component Gallery is missing foundation example: $GalleryFoundationToken"
}

...
if ($Contract.component_registry.version -ne "1.4.1" -or $Registry.registry_version -ne "1.4.1") {
    Add-ContractFailure "Content Contract must reference Component Registry 1.4.1."
}
...
if ($Contract.theme_version -ne "0.33.6") {
    Add-ContractFailure "Content Contract theme_version must be 0.33.6."
}
...
if (Test-Path -LiteralPath $ThemeFunctionsPath -PathType Leaf) {
    ...
    if (-not $ThemeFunctions.Contains("inc/commercial-events.php")) {
        Add-ContractFailure "Parent Theme does not load the Commercial Event relay."
    }
}
